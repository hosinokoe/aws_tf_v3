...
resource "aws_s3_bucket_acl" "s3_pri" {
  # for_each = toset(var.s3_private)
  # bucket = aws_s3_bucket.s3_pri[each.key].id
  for_each = aws_s3_bucket.s3_pri
  bucket = each.key
  acl    = "public-read"
}

resource "aws_s3_bucket_policy" "s3_pri_acl" {
  for_each = aws_s3_bucket.s3_pri
  bucket = each.key
  policy = jsonencode({
    Version = "2012-10-17"
...
resource "aws_iam_user" "s3_uploads" {
  for_each = toset(concat(var.s3_private,var.s3_public))
  name = each.value
  path = "/users/"
}
