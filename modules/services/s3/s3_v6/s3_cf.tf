# s3_v1/s3_v2均无zabbix，本代码有zabbix，cf证书为域名证书，从v3复制过来，增加s3权限，ssl证书由2019改为2021，fs_pro专用，添加cors
    ...
    Statement = [
      {
        ...
        Sid       = "2"
        Effect    = "Allow"
        Principal = {AWS = "arn:aws:iam::cloudfront:user/CloudFront Origin Access Identity ${var.cf_acl}"}
        Action    = "s3:GetObject"
...
resource "aws_cloudfront_distribution" "s3_distribution" {
  ...
  origin {
    ...
    s3_origin_config {
     origin_access_identity = "origin-access-identity/cloudfront/${var.cf_acl}"
    }
  }
